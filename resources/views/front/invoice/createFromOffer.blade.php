                ...
                <div class="widget-bg">
                    <div class="widget-body clearfix">
                        <form id="invoiceForm" action="{{ route('invoice.store', ['id' => $data['id']]) }}" method="POST"
                            ...
                            @include('front.invoice.invoiceFromOfferComponents.invoiceUmzug2',['umzug' => $umzug])
                            {{-- Invoice Umzug 2 Alanı --}}

                            {{-- Invoice Einpackservice Alanı --}}
                            @include('front.invoice.invoiceFromOfferComponents.invoiceEinpack',['einpack' => $einpack])
                            {{-- Invoice Einpackservice Alanı --}}

                            {{-- Invoice Auspackservice Alanı --}}
                            @include('front.invoice.invoiceFromOfferComponents.invoiceAuspack',['auspack' => $auspack])
                            {{-- Invoice Auspackservice Alanı --}}

                            {{-- Invoice Reinigung Alanı --}}
                            @include('front.invoice.invoiceFromOfferComponents.invoiceReinigung',['reinigung' => $reinigung])
                            {{-- Invoice Reinigung Alanı --}}

                            {{-- Invoice Reinigung2 Alanı --}}
                            @include('front.invoice.invoiceFromOfferComponents.invoiceReinigung2',['reinigung2' => $reinigung2])
                            {{-- Invoice Reinigung2 Alanı --}}

                            {{-- Invoice Entsorgung Alanı --}}
                            @include('front.invoice.invoiceFromOfferComponents.invoiceEntsorgung',['entsorgung' => $entsorgung])
                            {{-- Invoice Entsorgung Alanı --}}

                            {{-- Invoice Transport Alanı --}}
                            @include('front.invoice.invoiceFromOfferComponents.invoiceTransport',['transport' => $transport])
                            {{-- Invoice Transport Alanı --}}

                            {{-- Invoice Lagerung Alanı --}}
                            @include('front.invoice.invoiceFromOfferComponents.invoiceLagerung',['lagerung' => $lagerung])
                            {{-- Invoice Lagerung Alanı --}}

                            {{-- Invoice Lagerung Alanı --}}
                            @include('front.invoice.invoiceFromOfferComponents.invoiceMaterial',['material' => $material, 'basket' => $basket])
                            {{-- Invoice Lagerung Alanı --}}
...
@yield('invoiceOfferUmzug')
@yield('invoiceOfferEinpack')
@yield('invoiceOfferAuspack')
@yield('invoiceOfferFooterReinigung')
@yield('invoiceOfferFooterReinigung2')
@yield('invoiceOfferEntsorgung')
@yield('invoiceOfferFooterTransport')
@yield('invoiceOfferFooterLagerung')
@yield('invoiceOfferMaterial')
@endsection
